...
/*   Updated: 2022/04/15 00:31:28 by nfaivre          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	PhoneBook::add(void)
{
	if (this->amount >= this->max_amount)
	{
		...
		std::cout << "# The oldest contact will be replaced by the new one #" << std::endl;
	}
...
void	PhoneBook::display_search_header(void)
{
	...
	std::cout << "||" << std::setw(Contact::preview_column_width) << "index" << '|';
...
void	PhoneBook::search(void) const
{
	...
	{
		std::cout << "# Type an index between 0 and " << (((this->amount < this->max_amount) ? (this->amount) : (this->max_amount)) - 1) << " #" << std::endl;
		std::cout << "> ";
